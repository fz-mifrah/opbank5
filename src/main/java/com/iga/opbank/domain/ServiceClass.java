...
public class ServiceClass implements Serializable {
    ...
    @ManyToMany(mappedBy = "serviceClasses")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "serviceClasses", "operation" }, allowSetters = true)
    ...
    public void setPaimentFactures(Set<PaimentFacture> paimentFactures) {
        if (this.paimentFactures != null) {
            this.paimentFactures.forEach(i -> i.removeServiceClass(this));
        }
        if (paimentFactures != null) {
            paimentFactures.forEach(i -> i.addServiceClass(this));
        }
