...
export class PaimentFactureUpdateComponent implements OnInit {
  ...
  getSelectedServiceClass(option: IServiceClass, selectedVals?: IServiceClass[]): IServiceClass {
    if (selectedVals) {
  ...
  protected updateForm(paimentFacture: IPaimentFacture): void {
    this.editForm.patchValue({
      ...
      serviceClasses: paimentFacture.serviceClasses,
    });
    ...
    this.serviceClassService
      ...
      .pipe(
        map((serviceClasses: IServiceClass[]) =>
          this.serviceClassService.addServiceClassToCollectionIfMissing(
            serviceClasses,
            ...(this.editForm.get('serviceClasses')!.value ?? [])
          )
        )
      )
  ...
  protected createFromForm(): IPaimentFacture {
    return {
      ...
      serviceClasses: this.editForm.get(['serviceClasses'])!.value,
    };
  }
}
